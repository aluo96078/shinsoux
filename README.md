...
- Windows snapshot 與內容位於 `%LOCALAPPDATA%\ShinsouXData`；敏感 KV 同樣使用 AES-256-GCM，master key 只以目前使用者範圍 DPAPI protected blob 落盤。舊版 `%LOCALAPPDATA%\Shinsou X` 會在首次啟動時遷移至新目錄。
